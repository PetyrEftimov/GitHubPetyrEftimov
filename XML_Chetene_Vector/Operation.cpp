...
void Operation::readFromVectorWriteToBinFile(){
    ...
    for (pugi::xml_node cd = nodeObjekt.child("CD");cd;cd = cd.next_sibling("CD")) {
        ...
        Cd* cd1 = new Cd(cd.child("TITLE").text().as_string(),cd.child("ARTIST").text().as_string(),cd.child("PRICE").text().as_double());
//        
//        vecPtr->setTitle(cd.child("TITLE").text().as_string());
//        vecPtr->setArtist(cd.child("ARTIST").text().as_string());
//        vecPtr->setPrice(cd.child("PRICE").text().as_double());
//        
        v_cds.push_back(cd1);
...
void Operation:: test(){
    ...
    for (it = catalog.begin(); it != catalog.end(); ++it) {
       // cout << it -> child_value("TITLE");
        Cd cd2;
        cd2.setTitle(it -> child_value("TITLE"));
        cd2.setArtist(it -> child_value("ARTIST"));
        cd2.setPrice(it -> child("PRICE").text().as_double());
        v_cd.push_back(cd2);
        
        
    }
    
    vector<Cd>::iterator itr ;
    double allCdPrice = 0 ;
    for (itr = v_cd.begin(); itr != v_cd.end(); itr++) {
        itr -> cdInfo();
        allCdPrice = allCdPrice+itr -> getPrice();
